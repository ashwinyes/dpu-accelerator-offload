...
mempool_process(struct mempool_config *config)
{
	...
	mp = rte_pktmbuf_pool_create(config->name, config->pool_size, config->cache_size, 128,
				     config->buffer_size, config->numa_node);
	if (!mp)
		return -EINVAL;
