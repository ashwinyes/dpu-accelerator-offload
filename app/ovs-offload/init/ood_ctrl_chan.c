...
ood_control_channel_init(struct ood_main_cfg_data *ood_main_cfg)
{
	...
	uint16_t host_port, repr_qid;
	ood_repr_param_t *repr_prm;
	ood_ethdev_param_t *eth_prm;
	ood_config_param_t *cfg_prm;
	uint16_t *host_mac_lkp_tbl;
	int i, nb_port;
	...
	host_mac_lkp_tbl = eth_prm->dest_port_tbl_lkp;
	nb_port = cfg_prm->nb_port_pair_params;

	for (i = 0; i < nb_port; i++) {
		repr_qid = i;
		host_port = host_mac_lkp_tbl[i];
		rep_map[repr_qid].host_port = host_port;
		rep_map[repr_qid].mac_port = host_mac_lkp_tbl[host_port];
