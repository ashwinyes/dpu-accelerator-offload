...
static int
config_port_max_pkt_len(ood_config_param_t *cfg_prm, struct rte_eth_conf *conf,
...
port_init(struct ood_main_cfg_data *ood_main_cfg, uint16_t portid, uint16_t nb_lcores)
{
	...
	rc = config_port_max_pkt_len(ood_main_cfg->cfg_prm, &local_port_conf, &dev_info);
	if (rc != 0)
	...
	printf(", ");
	rte_eth_macaddr_get(ood_main_cfg->eth_prm->dest_port_tbl_lkp[portid],
...
ood_ethdev_init(struct ood_main_cfg_data *ood_main_cfg)
{
	...
	/* reset dest_port_tbl_lkp */
	for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++)
		eth_prm->dest_port_tbl_lkp[portid] = 0;
	last_port = 0;
	...
	if (cfg_prm->nb_port_pair_params) {
		...
		for (idx = 0; idx < (cfg_prm->nb_port_pair_params << 1); idx++) {
			p = idx & 1;
			portid = cfg_prm->port_pair_param[idx >> 1].port[p];
			eth_prm->dest_port_tbl_lkp[portid] =
				cfg_prm->port_pair_param[idx >> 1].port[p ^ 1];
		}
	} else {
		...
		RTE_ETH_FOREACH_DEV(portid) {
			...
			if (nb_ports_in_mask % 2) {
				eth_prm->dest_port_tbl_lkp[portid] = last_port;
				eth_prm->dest_port_tbl_lkp[last_port] = portid;
			} else {
				last_port = portid;
			}
		...
		if (nb_ports_in_mask % 2) {
			...
			eth_prm->dest_port_tbl_lkp[last_port] = last_port;
		}
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		dao_info("Port ID %d (%s) -> Dest Port ID %d", portid, if_name,
			 eth_prm->dest_port_tbl_lkp[portid]);
	}
	eth_prm->nb_ports = i;
	...
	RTE_ETH_FOREACH_DEV(portid) {
		if (portid == ood_repr_get_eswitch_portid(ood_main_cfg))
			continue;
		ood_main_cfg->graph_prm->fm_ctrl_cfg.host_mac_map[portid] =
			eth_prm->dest_port_tbl_lkp[portid];

		if (portid < cfg_prm->nb_port_pair_params)
			ood_main_cfg->graph_prm->fm_ctrl_cfg.host_ports[portid] =
				eth_prm->dest_port_tbl_lkp[portid];
	}
