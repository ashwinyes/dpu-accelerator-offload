...
configure_eswitch_dev(ood_repr_param_t *repr_prm, uint16_t port_id)
{
	...
	nb_rxq = 1;
	nb_txq = 1;
	rc = rte_eth_dev_configure(port_id, nb_rxq, nb_txq, &port_conf);
	if (rc != 0)
...
ood_representor_eswitch_dev_init(struct ood_main_cfg_data *ood_main_cfg)
{
	...
	dao_info("Representor eswitch device port ID %d, name %s", portid,
		 name);

	/* configure eswitch_dev */
	if (configure_eswitch_dev(repr_prm, portid))
