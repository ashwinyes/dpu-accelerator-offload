...
* Populates the virtio capabilities to be availble to the host as per virtio specification.
* Registers ``struct virtio_pci_common_cfg`` bar area to the ``pem library`` using
  ``dao_pem_ctrl_region_register()`` to get notifications when host chages in this area.

Since this is base virtio device, its initilization is called as part of higher level device
