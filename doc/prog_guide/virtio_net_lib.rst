...
Virtio-net library is the virtulization solution used in CN10K for networking.
This model emulates SMART NICs for VM and frontend virtio network driver.
...
* Using VIRTIO_F_ORDER_PLATFORM is manadatory for proper functioning of smart NIC as
...
The initialization of each virtio net device inlcudes the following operations:

* Initialize base virtio device using ``virtio_dev_init()`` API, which populates the virtio
  capabilities to be avaialble to host.
...
The application is expected to register callbacks to take the appropriate actions for each control command. Virtio net library triggers the corresponding callaback function when it recieves the control command.
