		  ...
		  const uint16_t flags)
{
	...
	uint16_t last_off = DESC_OFF(q->last_off);
	...
	while (i < total_mbufs) {
		...
		while (unlikely(dflags) && (segs == 1)) {
			...
			*((uint64_t *)&mbuf2->rearm_data) = rearm_data;
			mbuf0->pkt_len += mbuf2->data_len;
			mbuf0->nb_segs++;
			mbuf1 = mbuf2;
			i++;
		}
		if ((i >= total_mbufs) || (i + segs > total_mbufs))
			break;
		...
		const uint16_t flags)
{
	...
	while (i < nb_mbufs) {
		...
		*((uint64_t *)&mbuf->rearm_data) = rearm_data + sizeof(struct virtio_net_hdr);
		mbuf->pkt_len = len - sizeof(struct virtio_net_hdr);
		mbuf->data_len = len - sizeof(struct virtio_net_hdr);
		mbuf->next = NULL;
