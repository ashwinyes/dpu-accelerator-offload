		      ...
		      struct rte_mbuf *mbuf, uint16_t *qoff)
{
	...
	do {
		...
		buf_len = (d_flags & (RTE_BIT64(32) - 1)) - sizeof(struct virtio_net_hdr);
		...
		*DESC_PTR_OFF(sd_desc_base, off, 8) = avail << 55 | avail << 63 | d_flags |
		...
		dao_dma_enq_x1(mem2dev, (uintptr_t)hdr, len, *DESC_PTR_OFF(sd_desc_base, off, 0),
			       len);
	      ...
	      struct rte_mbuf **mbufs, uint16_t nb_mbufs, const uint16_t flags)
{
	...
	for (i = 0; i < count; ) {
		...
		if (flags & VIRTIO_NET_ENQ_OFFLOAD_CHECKSUM) {
			flags01 = vzip1q_u64(len_olflags0, len_olflags1);
			flags23 = vzip1q_u64(len_olflags2, len_olflags3);

			/* Pick lower 32 bits of olflags from each packet */
			ol_flags = vuzp1q_u32(flags01, flags23);
	...
	while (i < nb_mbufs) {
		...
		*DESC_PTR_OFF(sd_desc_base, off, 8) = avail << 55 | avail << 63 | d_flags |
		...
		if ((flags & VIRTIO_NET_ENQ_OFFLOAD_MSEG) && (nb_enq > 1))
			process_mseg_pkts_enq(q, mem2dev, (struct rte_mbuf *)mbuf0, &off);
