...
mark_deq_compl_fn_t mark_deq_compl_fns[2] = {mark_deq_compl_inorder, mark_deq_compl_no_inorder};
...
virtio_netdev_populate_queue_info(struct virtio_netdev *netdev)
{
	...
	for (i = 0; i < max_vqs; i++) {
		...
		/* Set function pointer for marking dequeue descriptors as used,
		 * according to the VIRTIO_F_IN_ORDER flag.
		 */
		if (dev->feature_bits & RTE_BIT64(VIRTIO_F_IN_ORDER))
			queue->mark_fn_id = VIRTIO_NET_DEQ_INORDER;
		else
			queue->mark_fn_id = VIRTIO_NET_DEQ_NOINORDER;
...
dao_virtio_net_desc_manage(uint16_t devid, uint16_t qp_count)
{
	...
	for (i = 0; i < qp_count; i++) {
		...
		src = dao_dma_sge_src(dev2mem);
		dst = dao_dma_sge_dst(dev2mem);
		sg_i = fetch_deq_desc_prep(q, dev2mem, src, dst);
		dev2mem->src_i += sg_i;
		dev2mem->dst_i += sg_i;
	}
	...
	for (i = 0; i < qp_count; i++) {
		...
		mark_deq_compl_fns[q->mark_fn_id](q, mem2dev, compl_off, nb_desc);
		q->compl_off = off;
	}
	...
	for (i = 0; i < qp_count; i++) {
		...
		mark_enq_compl(q, mem2dev, compl_off, off);
		q->compl_off = off;
...
void
mark_deq_compl_inorder(struct virtio_net_queue *q, struct dao_dma_vchan_state *mem2dev,
		       uint16_t start, uint16_t nb_desc)
{
	uintptr_t sd_desc_base = (uintptr_t)q->sd_desc_base;
	uintptr_t desc_base = q->desc_base;
